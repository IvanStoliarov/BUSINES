...
<?php if(isset($_SESSION['logged_user'])&&isset($_SESSION['choosen_performers'])){ ?>
    ...
    if($user_balance<$sum){
		...
		?></div><br><hr>
		<form id="order_to_payment" method="post" action="<?php if($amount_to_pay==0){ echo '../payment/proceed_from_balance.php'; }else{ echo '../payment_page.php'; } ?>"><input type="hidden" name="price" value="<?php echo $amount_to_pay ?>">
		...
		<button form="order_to_payment" class="go_to_payment_button">Оплатить <?php echo $amount_to_pay ?> руб.</button>
		</form>
	</div>
