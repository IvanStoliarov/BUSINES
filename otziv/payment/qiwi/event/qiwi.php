    ...
    // ЗДЕСЬ ЗАПИСЬ В БД ИНФОРМАЦИИ О ПЛАТЕЖЕ //
        ...
        if($json->sum < $_SESSION['post']['sum']){
            ...
            <script type="text/javascript">
                alert('Ваш ваучер стоит дешевле запрашиваемых <?php echo $_SESSION['post']['sum'] ?> руб. ваши деньги переведы на баланс!');
            </script>
            <?php
            exit();    
        }
        ...
        if($json->sum > $_SESSION['post']['sum']){
            mysqli_query($connection, "UPDATE `USERS` SET BALANCE='".mysqli_fetch_assoc(mysqli_query($connection, "SELECT * FROM `USERS` WHERE ID='".$_SESSION['logged_user']['ID']."'"))['BALANCE'] + ($json->sum - $_SESSION['post']['sum'])."' WHERE ID='".$_SESSION['logged_user']['ID']."'")
        }
